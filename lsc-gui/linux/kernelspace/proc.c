...
ssize_t lscpcie_read_proc(struct file *filp, char __user *buf, size_t count,
                          loff_t *offp) {
...
ssize_t lscpcie_write_proc(struct file *filp, const char __user *buf,
  ...
  for (i = 0; i < count; i++) {
	...
	continue;
      ...
      if (c != ':') goto reset; /* wrong character, back to square one */
      n_cameras = 0;
      continue;
    }
	...
	if (lscpcie_devices[slot].minor < 0) {
	  result = device_init(&lscpcie_devices[slot], slot);
	  if (result < 0) {
	    i = result; /* something unexpected happend, report the error */
	    break;
	  }
	  proc_init(&lscpcie_devices[slot]);
	  break;
	}
      if (slot == MAX_BOARDS) i = -ENOMEM; /* no free device left */
      goto reset;
    }
    /* is not line end, error */
    goto reset;
  }
                                    ...
                                    size_t count, loff_t *offp)
{
  ...
  while ((dev->proc_actual_register < 0x80)
    ...
    val = ioread8(dev->mapped_pci_base + dev->proc_actual_register);
...
ssize_t lscpcie_read_io_proc(struct file *filp, char __user *buf, size_t count,
                             loff_t *offp)
{
  ...
  if (!offset) {
    ...
    memcpy_fromio(&dma_data, dev->mapped_pci_base, sizeof(dma_reg_t));
    memcpy_fromio(&s0_data, dev->mapped_pci_base + 0x80, sizeof(s0_t));
  }
