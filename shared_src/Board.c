...
es_status_codes InitMeasurement(struct global_settings settings)
{
	...
	if (status != es_no_error) return status;
	//init Camera
	status = initCamera(settings.drvno, (UINT8)settings.camera_system, (UINT16)settings.pixel, (UINT16)settings.trigger_mode_cc, (UINT16)settings.sensor_type, (UINT8)settings.ADC_Mode, (UINT16)settings.ADC_custom_pettern, (UINT16)settings.led_off, (UINT16)settings.gain_switch, (UINT8)settings.gain_3030);
	if (status != es_no_error) return status;
	//set gain switch
